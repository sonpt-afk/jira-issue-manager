...
const App = () => {
    ...
    useEffect(() => {
        ...
        fetchProjects()
            .then(data => {
                const projectOptions = data.map(p => ({ label: p.name, value: p.key }));
    ...
    const getIssues = useCallback(async (projectKey, page) => {
        ...
        try {
            const { issues: fetchedIssues, total } = await fetchIssuesByProject(projectKey, startAt, ISSUES_PER_PAGE);
    ...
    const rows = issues.map((issue, index) => ({
        ...
        cells: [
            {
                key: 'type',
                content: <img src={issue.fields.issuetype.iconUrl} alt={issue.fields.issuetype.name} />,
            },
            ...
            },
            {
                ...
                content: issue.fields.assignee ? (
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        <Avatar src={issue.fields.assignee.avatarUrls['24x24']} size="small" />
                        <span style={{ marginLeft: 8 }}>{issue.fields.assignee.displayName}</span>
                    </div>
                ) : (
                    'Unassigned'
                ),
